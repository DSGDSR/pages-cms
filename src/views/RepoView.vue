            ...
            <ul role="list" class="flex flex-1 flex-col">
              <li>
                ...
                >
                    ...
                    >
                      ...
                      >
                        <Icon
                          :name="item.icon"
                          :fallback="
                            item.type == 'collection' ? 'FileStack' : 'FileText'
                          "
                  ...
                  <li
                    v-if="
                      repoStore.config?.object?.media != null &&
                      repoStore.config?.object?.media.input != null &&
                      repoStore.config?.object?.media.output != null
                    "
                  >
                    <router-link
                      :to="{ name: 'media' }"
        ...
        <template v-if="status == 'error-empty-repo'">
          <div class="error h-screen">
            <div class="text-center max-w-md">
              ...
              <p class="text-neutral-400 dark:text-neutral-500 mb-6">
                You need to add a
                <code
                  class="text-sm bg-neutral-100 dark:bg-neutral-850 rounded-lg p-1"
                  >.pages.yml</code
                >
                file to this repository and create a branch to configure it.
              </p>
              <div class="flex gap-x-2 justify-center">
                <button class="btn-primary" @click="createConfigFile()">
            ...
            <div class="text-center max-w-md">
              ...
              <p class="text-neutral-400 dark:text-neutral-500 mb-6">
                ...
                <code
                  ...
                  >.pages.yml</code
                >
                file to this repository/branch to configure it.
              </p>
              <div class="flex gap-x-2 justify-center">
                <button class="btn-primary" @click="createConfigFile()">
                  Create a configuration file
...
const repoStore = reactive({
  ...
  config: computed(
    () =>
      config.state[`${repoStore.owner}/${repoStore.repo}/${repoStore.branch}`]
  ),
  details: null,
});
...
const configValidationErrors = computed(() =>
  repoStore.config.validation.filter((entry) => entry.severity === "error")
);
...
const createConfigFile = async () => {
  ...
  if (data) {
    notifications.notify(
      `The configuration file (.pages.yml) was successfully created.`,
      "success"
    );
    await setRepo();
    status.value = "";
    ...
    notifications.notify(
      `The configuration file (.pages.yml) couldn't be created. Try reloading the page.`,
      "error",
      { delay: 0 }
    );
    status.value = "";
...
const setRepo = async () => {
  ...
  if (!repoDetails) {
    notifications.notify(
      `The repo "${repoStore.owner}/${props.repo}" doesn't exist.`,
      "error",
      { delay: 10000 }
    );
  ...
  if (repoBranches.length === 0) {
    ...
    router.push({
      name: "content-root",
      params: { ...route.params, branch: repoDetails.default_branch },
    });
    return;
  } else {
  ...
  if (!repoStore.config) {
    // No config file
    notifications.notify(
      "No configuration file (.pages.yml) in the repository.",
      "warning"
    );
    ...
    if (route.name !== "settings") {
      notifications.notify(
        "Your settings are empty, redirecting you to the settings page.",
        "warning"
      );
    ...
    const validationErrors = repoStore.config.validation.filter(
    ...
    if (validationErrors.length > 0) {
      ...
      if (route.name !== "settings") {
        ...
        notifications.notify(
          "Your settings are not valid, please fix errors.",
          "warning"
        );
      }
    } else {
      if (
        ["content", "content-root"].includes(route.name) &&
        !repoStore.config.object.content?.[0]
      ) {
        if (repoStore.config.object.media) {
          router.push({ name: "media" });
        } else {
          notifications.notify(
            "No settings for content, redirecting you to the settings page.",
            "warning"
          );
          router.push({ name: "settings" });
        }
      } else if (
        route.name === "content-root" &&
        repoStore.config.object.content?.[0]?.name
      ) {
        router.push({
          name: "content",
          params: { name: repoStore.config.object.content[0].name },
        });
      } else if (route.name === "media" && !repoStore.config.object.media) {
        notifications.notify(
          "No settings for media, redirecting you to the settings page.",
          "warning"
        );
        router.push({ name: "settings" });
      }
